...
const enviarCorreoOTP = async (destinatario, codigo) => {
  ...
  const mailOptions = {
    from: process.env.EMAIL_FROM || `"Consejo Comunal El Valle" <${process.env.EMAIL_USER}>`,
    to: destinatario,
    subject: 'Código de recuperación - Consejo Comunal El Valle',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 16px;">
        <div style="text-align: center; margin-bottom: 24px;">
          <div style="display: inline-flex; align-items: center; justify-content: center; width: 48px; height: 48px; background: #2563eb; border-radius: 12px; margin-bottom: 8px;">
            <span style="font-size: 24px;">🏘️</span>
          </div>
          <h1 style="font-size: 20px; font-weight: 800; color: #1e293b; margin: 0;">Consejo Comunal El Valle</h1>
        ...
        <p style="font-size: 12px; color: #64748b;">Este código expirará en 3 minutos. Si no solicitaste este cambio, ignora este correo.</p>
        <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 20px 0;" />
        <p style="font-size: 10px; color: #94a3b8; text-align: center;">© ${new Date().getFullYear()} Consejo Comunal El Valle</p>
      </div>
    `,
  };
