...
@Component({
  ...
  template: `
    <div class="max-w-2xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        <h2 class="text-2xl font-black text-slate-800 dark:text-white tracking-tight">{{ isEdit() ? 'Editar Familia' : 'Nueva Familia' }}</h2>
        <p class="text-sm text-slate-500 dark:text-slate-400 font-medium">Registra la información del núcleo familiar.</p>
      </div>
      ...
      <form (ngSubmit)="save()" class="bg-white dark:bg-slate-900/40 border border-slate-200/80 dark:border-slate-800/80 p-6 md:p-8 rounded-3xl shadow-sm space-y-6">
        <div class="grid grid-cols-1 gap-6">
          ...
          <div class="space-y-2">
            ...
            <input
              [(ngModel)]="form.nombre" name="nombre" required
          ...
          <div class="space-y-2">
            ...
            <input
              [(ngModel)]="form.direccion" name="direccion" required
              placeholder="Ej: Calle El Carmen, casa 4-B"
              class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 dark:focus:border-blue-500 transition-all font-medium"
            />
          </div>
          ...
          <div class="space-y-2">
            ...
            <select
              [(ngModel)]="form.consejoId" name="consejoId" required
...
export class FamiliaFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (id) {
      ...
      this.famSvc.getById(+id).subscribe((r) => {
  ...
  save() {
    ...
    obs.subscribe({
      next: () => { this.saving.set(false); this.notify.success(id ? 'Familia actualizada' : 'Familia creada', 'La familia se ha guardado correctamente.'); this.router.navigate(['/app/familias']); },
      error: (e) => { this.error.set(e?.error?.message ?? 'Error.'); this.saving.set(false); this.notify.error('Error', e?.error?.message ?? 'Error al guardar la familia.'); },
    });
  }

  cancel() { this.router.navigate(['/app/familias']); }
}
