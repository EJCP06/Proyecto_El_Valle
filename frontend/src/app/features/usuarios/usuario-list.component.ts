...
export class UsuarioListComponent implements OnInit {
  ...
  save() {
    ...
    const payload = id ? this.form : { ...this.form, password: this.password };
    const obs = id ? this.svc.update(id, this.form) : this.svc.create(payload as Partial<Usuario> & { password: string });
    obs.subscribe({
      next: () => {
        this.saving.set(false);
